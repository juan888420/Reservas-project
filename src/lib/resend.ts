  ...
  const result = await getResend().emails.send({
    ...
    html: `
      <div style="font-family: system-ui, sans-serif; max-width: 520px; margin: 0 auto; color: #e8e8e8; background: #111; padding: 32px; border-radius: 12px;">
        ...
        <p style="color: #888; margin: 0 0 24px;">Hola ${escapeHtml(params.pacienteNombre)}, tu reserva ha sido confirmada.</p>
        <table style="width: 100%; border-collapse: collapse;">
          <tr><td style="padding: 8px 0; color: #888;">Médico</td><td style="padding: 8px 0; text-align: right;">${escapeHtml(params.medicoNombre)}</td></tr>
          <tr><td style="padding: 8px 0; color: #888;">Especialidad</td><td style="padding: 8px 0; text-align: right;">${escapeHtml(params.especialidad)}</td></tr>
          <tr><td style="padding: 8px 0; color: #888;">Fecha</td><td style="padding: 8px 0; text-align: right;">${fechaFmt}</td></tr>
          <tr><td style="padding: 8px 0; color: #888;">Hora</td><td style="padding: 8px 0; text-align: right;">${escapeHtml(params.hora.slice(0, 5))}</td></tr>
          ${params.motivo ? `<tr><td style="padding: 8px 0; color: #888;">Motivo</td><td style="padding: 8px 0; text-align: right;">${escapeHtml(params.motivo)}</td></tr>` : ""}
